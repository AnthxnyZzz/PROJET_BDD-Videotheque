package view;

import java.awt.BorderLayout;
import java.awt.Font;

import javax.swing.JButton;
import javax.swing.JLabel;
import javax.swing.JPanel;
import javax.swing.JScrollPane;
import javax.swing.JTextArea;

import controllers.BoutonRecherchereal;

public class EcranRechercheReal extends JPanel {
	private static final long serialVersionUID = 1L;
	private JLabel tag;
	private JLabel tag1;
	private JLabel tag2;
	private JPanel milieu;
	private JTextArea nomreal;
	private JTextArea prenomreal;
	private JButton rechercher;
	private JTextArea resultat;
	private JScrollPane sp;  
	private JPanel contenuresultat;
	
	public EcranRechercheReal() {
		// TODO Auto-generated constructor stub
		super();
		this.setLayout(new BorderLayout());
		
		tag=new JLabel ("Rechercher un réalisateur");
		tag.setFont(tag.getFont().deriveFont(Font.PLAIN, 19));
		
		tag1=new JLabel ("Nom : ");
		nomreal=new JTextArea(1,15);
		tag2=new JLabel ("Prénom : ");
		prenomreal=new JTextArea(1,15);
		rechercher= new JButton("Rechercher");
		resultat=new JTextArea(16,52);
		resultat.setEditable(false);
		
		sp= new JScrollPane(resultat);
		
		contenuresultat=new JPanel();
		contenuresultat.add(sp);
		milieu=new JPanel();
		milieu.add(tag1);
		milieu.add(nomreal);
		milieu.add(tag2);
		milieu.add(prenomreal);
		milieu.add(rechercher);
		
		add(tag,BorderLayout.NORTH);
		add(milieu,BorderLayout.CENTER);
		add(contenuresultat, BorderLayout.SOUTH);
		
		
		rechercher.addActionListener(new BoutonRecherchereal(this));
	}

	public JTextArea getNomreal() {
		return nomreal;
	}

	public void setNomreal(JTextArea nomreal) {
		this.nomreal = nomreal;
	}

	public JTextArea getPrenomreal() {
		return prenomreal;
	}

	public void setPrenomreal(JTextArea prenomreal) {
		this.prenomreal = prenomreal;
	}

	public JTextArea getResultat() {
		return resultat;
	}

	public void setResultat(JTextArea resultat) {
		this.resultat = resultat;
	}

}
